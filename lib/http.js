...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/account/balance', async (req, res) => {
      ...
      // HACK: getCoinsByAddress requires an Address instance or list, however
      // we cannot pass those from here as they'd be a different object. Since
      // RPC doesn't have any way to return coins by address, querying them
      // through the HTTP API is our only way :/
      // See: https://github.com/handshake-org/hsd/pull/466
      // TODO: This hack assumes no config options are passed to hsd... either
      // error when --rpc-port are not default or use config options here.
      const result = await new brq(`http://localhost:${this.network.rpcPort}/coin/address/${address}`);
      const coins = result.json();

      // TODO: Check coins schema: [{'value': 10,..}]
      if (coins.error) {
        res.json(500, {
          code: -5,
          message: coins.error.message,
          retriable: false
        });
        return;
      }
